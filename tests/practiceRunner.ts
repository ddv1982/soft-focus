...
const runOvershootCarryScenario = (): void => {
  ...
  assert(
    afterBoundaryOvershoot.phase === 'phrase',
    'expected overshoot to advance into the next phase',
  );
  assert(afterBoundaryOvershoot.phaseIndex === 1, 'expected phase index to advance once');
  assert(
    afterBoundaryOvershoot.secondsRemaining === 2,
    'expected carried 500ms overshoot to reduce the next phase',
  );
  assert(
    afterBoundaryOvershoot.totalSecondsRemaining === 3,
    'expected total remaining seconds to include carried overshoot',
  );

  const afterMultiBoundaryOvershoot = runner.tick(2000);

  assert(
    afterMultiBoundaryOvershoot.phase === 'recovery',
    'expected large overshoot to carry through multiple phase boundaries',
  );
  assert(
    afterMultiBoundaryOvershoot.phaseIndex === 2,
    'expected phase index to advance through the practice phase',
  );
  assert(
    afterMultiBoundaryOvershoot.secondsRemaining === 1,
    'expected remaining overshoot to apply to recovery',
  );
  assert(
    afterMultiBoundaryOvershoot.totalSecondsRemaining === 1,
    'expected total remaining to reflect recovery only',
  );
...
const runZeroDurationCarryScenario = (): void => {
  ...
  assert(
    afterZeroLengthPhases.phase === 'recovery',
    'expected positive delta to skip zero-duration phases',
  );
  assert(
    afterZeroLengthPhases.phaseIndex === 2,
    'expected runner to advance through both zero-duration phases',
  );
  assert(
    afterZeroLengthPhases.secondsRemaining === 1,
    'expected carried delta to apply to recovery phase',
  );
  assert(
    afterZeroLengthPhases.totalSecondsRemaining === 1,
    'expected total remaining to reflect carried delta in recovery',
  );
  ...
  assert(
    complete.complete,
    'expected follow-up tick to prove the 250ms overshoot was applied to recovery',
  );
};
...
const runInitialZeroDurationNormalizationScenario = (): void => {
  ...
  assert(
    initial.phaseIndex === 1,
    'expected constructor normalization to advance to the first positive phase',
  );
  assert(
    initial.secondsRemaining === 1,
    'expected first positive phase to retain its full duration',
  );
};
...
const runTickZeroNormalizationScenario = (): void => {
  ...
  assert(
    runner.getSnapshot().paused,
    'expected paused zero-duration snapshot to remain paused before a public transition',
  );
  runner.resume();
  const afterTickZero = runner.tick(0);

  assert(
    afterTickZero.phase === 'phrase',
    'expected tick(0) after resume to normalize zero-duration phase',
  );
  assert(
    afterTickZero.phaseIndex === 1,
    'expected tick(0) normalization to advance to the first positive phase',
  );
};
...
const runExactBoundaryZeroDurationScenario = (): void => {
  ...
  assert(
    afterExactBoundary.phase === 'recovery',
    'expected exact-boundary tick to skip zero-duration phase',
  );
  assert(
    afterExactBoundary.phaseIndex === 2,
    'expected exact-boundary tick to advance through zero-duration phase',
  );
  assert(
    afterExactBoundary.secondsRemaining === 1,
    'expected recovery to start with full duration after exact-boundary skip',
  );
  assert(
    afterExactBoundary.totalSecondsRemaining === 1,
    'expected total remaining to reflect recovery phase only',
  );
};
...
const runEmptyPhaseScenario = (): void => {
  ...
  assert(
    snapshot.totalSecondsRemaining === 0,
    'expected empty phase arrays to have no remaining time',
  );
};
...
const runAllZeroPhaseScenario = (): void => {
  ...
  assert(
    ...
    'expected all-zero phase arrays to report the complete phase',
  );
  assert(
    snapshot.phaseIndex === phases.length,
    'expected all-zero phase arrays to advance through every phase',
  );
  assert(
    snapshot.totalSecondsRemaining === 0,
    'expected all-zero phase arrays to have no remaining time',
  );
};
