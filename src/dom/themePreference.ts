...
export const getEffectiveThemePreference = (
  storage: StorageLike | null = getBrowserStorage(),
): ThemePreference => readStoredThemePreference(storage) ?? getSystemTheme();
...
export const setThemePreference = (
  ...
  if (typeof window !== 'undefined') {
    window.dispatchEvent(
      new CustomEvent<ThemePreference>('soft-focus:themechange', { detail: theme }),
    );
  }
  return theme;
};
...
export const toggleThemePreference = (
  ...
  const nextTheme: ThemePreference =
    getEffectiveThemePreference(storage) === 'dark' ? 'light' : 'dark';
